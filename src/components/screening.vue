  ...
  .screening-mask(v-if="show" @click.self="close")
    .screening
      .screening-content(v-if="!loading")
        ...
        .screening-item(v-if="form.first_id && hasBrand")
          .screening-label 品牌
          .screening-bd
            .screening-tag(@click="selectBrandAll(brands)" :class="{'active': !form.brand_id}") 全部
            .screening-tag(v-for="item in brandList", :class="{'active': form.brand_id == item.id}" @click="selectBrand(item)") {{item.name}}
...
export default {
  ...
  computed: {
    ...mapState({
      show: state => state.modal.screening.show,
      tagType: state => state.modal.screening.tagType,
      hasBrand: state => state.modal.screening.hasBrand,
      brandId: state => state.modal.screening.brandId
    })
  },
  methods: {
    ...mapActions([
    ...
    selectBrand (item) {
      this.$set(this.form, 'brand_id', item.id)
      // this.$router.replace({ name: this.$route.name, query: {...this.$route.query, brand_id: item.id} })
    },
    selectBrandAll (item) {
      this.$set(this.form, 'brand_id', '')
      // let query = this.$route.query
      // delete query['brand_id']
      // this.$router.replace({name: this.$route.name, query})
    },
    selectTag (item, label) {
      if (this.tagType === 'even') {
        item.isActive = !item.isActive
        if (label === '风格') {
          let index = this.styleList.findIndex(name => name === item.name)
          if (item.isActive && index < 0) {
            this.styleList.push(item.name)
          } else if (!item.isActive && index >= 0) {
            this.styleList.splice(index, 1)
          }
          this.tagList[0].selectedAll = !this.styleList.length
        } else {
          let index = this.spaceList.findIndex(name => name === item.name)
          if (item.isActive && index < 0) {
            this.spaceList.push(item.name)
          } else if (!item.isActive && index >= 0) {
        ...
        if (label === '风格') {
          this.tagList[0].tags.forEach(row => row.isActive = false)
          item.isActive = true
          this.styleList = [item.name]
          this.tagList[0].selectedAll = !this.styleList.length
        } else {
          this.tagList[1].tags.forEach(row => row.isActive = false)
          item.isActive = true
          this.spaceList = [item.name]
          this.tagList[1].selectedAll = !this.spaceList.length
        }
      }
    },
    ...
    confirm () {
      ...
      let filter = ['style_words', 'space_words', 'first_id', 'second_id', 'three_id', 'brand_id', 'goods_from']
      let query = {}
      filter.forEach(key => {
    ...
    reset () {
      ...
      let query = {}
      if (this.$route.query.good_type) query.good_type = this.$route.query.good_type
      if (this.$route.query.goods_from) query.goods_from = this.$route.query.goods_from
    ...
    async init () {
      this.loading = true
      const { style_words = '', space_words = '', first_id = '', second_id = '', three_id = '', brand_id = '', goods_from = '' } = this.$route.query
      this.form = { style_words, space_words, first_id, second_id, three_id, brand_id, goods_from }
      if (!style_words) {
      ...
      if (first_id && !this.brandList.length) this.getBrandList()
      this.getCategories()
