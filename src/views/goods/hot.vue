...
.hot-goods
  .slidebar
    .search
      ...
      input.search-input(:placeholder="$route.query.goods_from === 'shop'  ? '仅搜索本地馆' : '搜索'" v-model="search_words")
      .search-focus-line
      .search-line
    .menu
  ...
  infinite-scroll.goods(:loadMore="loadMore" ref="scroll")
    good-item(v-for="(item,index) in goods", :item="item" :key="item.goods_id")
...
<script>
import { hotGoods, getGoodsList, searchGoods, getStyleList, getSpaceList, getCategoryAndSon } from 'api'
...
export default {
  ...
  methods: {
    ...
    showScreening () {
      this.changeScreening({show: true, hasBrand: true})
    },
    async getGoodsList () {
      try {
        this.currentPage = 1
  ...
  beforeRouteEnter (to, from, next) {
    next(vm => {
      if (vm.$route.meta.scrollTop && from.name === 'goodDetail') {
        ...
        vm.getGoodsList()
        vm.$watch('search_words', (v) => {
          clearTimeout(timer)
          timer = setTimeout(() => {
            vm.$refs.scroll.$el.scrollTop = 0
            vm.$router.replace({ name: vm.$route.name, query: { ...vm.$route.query, search_words: v } })
          }, 1000)
        })
      }
    })
  ...
  beforeRouteUpdate (to, from, next) {
    next()
    this.search_words = to.query.search_words || ''
