<template lang="pug">
  ...
  transition(name="scale")
    infinite-scroll.good-detail(v-show="skuDetailShow", :loadMore="loadMore" ref="goods")
        ...
        .brand-cates(v-if="brandCates.length")
          ...
          .brand-cate(v-for="item in brandCates" :class="{'active': brandCate === item.id}" @click="selectBrandCate(item.id)")
            touch-ripple(:speed="3", :opacity=".6")
              span {{item.name}}
        .goods-list
          .goods-body(v-for="item in brandGoods" v-if="skuId !== item.goods_id")
...
export default {
  ...
  created () {
    if (this.skuId) this.getGoodDetail()
  },
  computed: {
    ...mapState({
      skuDetailShow: state => state.sketch.skuDetail.show,
      skuId: state => state.sketch.skuDetail.id,
      skuKey: state => state.sketch.skuDetail.key,
